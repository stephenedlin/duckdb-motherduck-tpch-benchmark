...
=============================================================
 ...
 OPTIONAL — run specific scale factors only:
   python motherduck_benchmark_v4.py --sf 1 5
   python motherduck_benchmark_v4.py --sf 10 20
   python motherduck_benchmark_v4.py --no-drop   (keep DB for debugging)

 OUTPUT:
   results/motherduck_results.csv   — all benchmark results
   results/logs/benchmark_*.log     — detailed session log
=============================================================
"""
...
def setup_logging(results_dir: str, scale_factors: list) -> str:
    log_dir   = os.path.join(results_dir, "logs")
    os.makedirs(log_dir, exist_ok=True)

    sf_tag    = "sf" + "_".join(str(s) for s in scale_factors)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_path  = os.path.join(log_dir, f"benchmark_{sf_tag}_{timestamp}.log")
...
def run_query(con, query_name: str, query_info: dict) -> dict:
    sql = query_info["sql"]
...
def benchmark_sf(con, sf: int, completed: set, rtt_ms: float) -> int:
    ...
    for q_num, (query_name, query_info) in enumerate(QUERIES.items(), start=1):
        ...
        logging.info(f"  [{q_num:02d}/{len(QUERIES)}] {query_name} [{query_info['category']}] — {query_info['tpch_ref']}")

        try:
            result = run_query(con, query_name, query_info)
            append_result(OUTPUT_CSV, query_name, query_info, sf, result, rtt_ms)
            completed.add(key)
            done += 1
...
if __name__ == "__main__":
    ...
    log_path = setup_logging(RESULTS_DIR, scale_factors)

    banner("MotherDuck Benchmark v4 — Stephen Edlin (PRADITA University)")
    ...
    logging.info(f"  Runs per query : {N_RUNS}")
    ...
    if completed:
        ...
        logging.info(f"  Resume mode: {already} queries for SF{scale_factors} already in CSV.\n")
    ...
    for sf in scale_factors:
        ...
        done_count = benchmark_sf(con, sf, completed, rtt_ms)
